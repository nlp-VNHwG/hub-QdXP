...
def get_city_weather(
    city_name: Annotated[str, "The Pinyin of the city name (e.g., 'beijing' or 'shanghai')"],
):
    """Retrieves the current weather data using the city's Pinyin name."""
    try:
        return requests.get(f"https://whyta.cn/api/tianqi?key={TOKEN}&city={city_name}").json()[
            "data"
        ]
    except Exception:
        return []
...
def get_address_detail(address_text: Annotated[str, "City Name"]):
    ...
    try:
        return requests.get(
            f"https://whyta.cn/api/tx/addressparse?key={TOKEN}&text={address_text}"
        ).json()["result"]
    except Exception:
        return []
...
def get_tel_info(tel_no: Annotated[str, "Tel phone number"]):
    ...
    try:
        return requests.get(
            f"https://whyta.cn/api/tx/mobilelocal?key={TOKEN}&phone={tel_no}"
        ).json()["result"]
    except Exception:
        return []
...
def get_scenic_info(scenic_name: Annotated[str, "Scenic/tourist place name"]):
    ...
    try:
        return requests.get(f"https://whyta.cn/api/tx/scenic?key={TOKEN}&word={scenic_name}").json()[
            "result"
        ]["list"]
    except Exception:
        return []
...
def get_flower_info(flower_name: Annotated[str, "Flower name"]):
    ...
    try:
        return requests.get(f"https://whyta.cn/api/tx/huayu?key={TOKEN}&word={flower_name}").json()[
            "result"
        ]
    except Exception:
        return []
...
def get_rate_transform(
    source_coin: Annotated[
        str, "The three-letter code (e.g., USD, CNY) for the source currency."
    ],
